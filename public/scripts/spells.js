...
const invulnerabilityBuffs = {
	498: true, 5573: true, 642: true, 1020: true, // Divine Shield
	1022: true, 5599: true, 10278: true, // Blessing of Protection
	11958: true, // Ice Block
	3169: true, // Limited Invulnerability Potion
	19752: true, // Divine Intervention
	6724: true, // Light of Elune
}
...
const threatFunctions = {
	sourceThreatenTarget(ev, fight, amount, useThreatCoeffs = true) {
		let a = fight.eventToUnit(ev, "source");
		let b = fight.eventToUnit(ev, "target");
		if (!a || !b) return;
		let coeff = useThreatCoeffs ? a.threatCoeff(ev.ability.type) : 1;
	...
	unitLeaveCombat(ev, unit, fight, text) {
		...
		if (!u) return;
		for (let k in enemies) {
			enemies[k].setThreat(u.key, 0, ev.timestamp, text);
		}
	},
...
function handler_modDamage(multiplier) {
	return (ev, fight) => {
		if (ev.type !== "damage") return;
		threatFunctions.sourceThreatenTarget(ev, fight, multiplier * (ev.amount + ev.absorbed));
	}
}
...
function handler_fixate(ev, fight) {
	if (ev.type !== "applydebuff" && ev.type !== "removedebuff") return;
	let a = fight.eventToUnit(ev, "source");
	let b = fight.eventToUnit(ev, "target");
	if (!a || !b) return;
	if (!("threat" in b)) return;
	let t = b.checkTargetExists(a.key, ev.timestamp);
	if (ev.type === "applydebuff") {
		t.fixates[ev.ability.name] = true;
		t.addMark(ev.timestamp, ev.ability.name);
	} else {
		t.addMark(ev.timestamp, ev.ability.name + " fades"); // This mark will still appear colored
...
// From ResultsMayVary https://resultsmayvary.github.io/ClassicThreatPerSecond/
        ...
        //Mocking Blow
        20560: threatFunctions.concat(handler_fixate, handler_damage, handler_markSourceOnMiss(borders.taunt)), //("Mocking Blow"),
        ...
        /* Zero threat abilities */
         355: threatFunctions.concat(handler_fixate, handler_taunt, handler_markSourceOnMiss(borders.taunt)), //("Taunt"), //Taunt
        1161: threatFunctions.concat(handler_markSourceOnMiss(borders.taunt), handler_fixate), //("Challenging Shout"), //Challenging Shout
        2687: handler_energize, //("Bloodrage"), //Bloodrage (cast)
       ...
       12976: handler_zero, //("Last Stand (buff)"), //Last Stand (buff)
        ...
        /* Bear */
5209: threatFunctions.concat(handler_markSourceOnMiss(borders.taunt), handler_fixate), // Challenging Roar
         ...
         6795: threatFunctions.concat(handler_fixate, handler_taunt, handler_markSourceOnMiss(borders.taunt)), //("Growl"),
